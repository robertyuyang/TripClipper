...
- **拍摄时段分组**：优先使用 DJI 文件名、视频内嵌 `creation_time` 或图片 EXIF
  拍摄时间划分 session，缺失时回退文件修改时间，方便按行程或场景整理素材。
...
- **失败追踪**：项目级失败记录发生时间，并在同阶段重跑时用本次结果替换旧记录。

### 4. 智能选片

- **Brief 驱动任务选片**：通过本机 Codex 读取 Markdown Brief、自主浏览索引和按需查看原视频帧，生成可修订的主选、备选与待人工复核候选池；结果和共享帧独立保存在项目的 `selections/` 目录。
...
- **组合筛选**：在 `review.html` 中按星级、状态、语音质量、相似组、候选池和
  session 等条件精确筛选，session 标题同步显示匹配数与总数。
...
Agent 会先浏览全部素材摘要，再按任务中必要分类批量打开完整索引详情；单次可用 `asset_get_batch` 检查 1～10 个素材。完整检查数量会随素材总数和分类数动态提高，避免只精读少量素材就下结论。索引证据不足时，Agent 可对指定素材和时间范围按需取帧；已有索引帧和 `selections/shared_frames/` 中的共享帧会优先复用，不足时才从只读原视频追加抽取。

深入比较后，Agent 可以更新或移除早期候选，并用审计事件保留移除原因。候选池支持 `primary`、`alternate` 和 `needs_review`；备选记录明确替代关系，待人工复核项记录保留价值、不确定点和人工查看重点。
...
其中 `T` 是 Brief 中的目标时长。例如 30 秒任务会形成 30～45 秒的主选池，供后续人工确认和剪辑编排。
