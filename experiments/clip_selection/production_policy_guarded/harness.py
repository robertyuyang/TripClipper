...
from experiments.clip_selection.runtime import model_context, structure_fingerprint

from .agent import SYSTEM_PROMPT
from .policy import SelectionPolicy
from .store import PolicyGuardedSelectionStore
from .tools import apply_action


class PolicyGuardedSelectionHarness:
    def __init__(
        self,
        *,
    ...
    def resume(self, request: SelectionRequest) -> SelectionRun:
        snapshot = ProjectSnapshot.from_cut_index(request.cut_index_path)
        store = PolicyGuardedSelectionStore(request.output_path)
        run = store.load()
    ...
    def _execute(
        ...
        while run.rounds < request.budget.max_rounds:
            ...
            if (
                ...
                return run
            try:
                should_finish, errors = apply_action(
                    run,
                    decision.action,
                    snapshot,
                    self.frame_source,
                ...
                should_finish, errors = False, [str(exc)]
            if errors:
