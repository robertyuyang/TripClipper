...
from experiments.clip_selection.runtime import validate_clip_reference
...
def apply_action(
    ...
    if action.name == "inspect_range":
        ...
        if start < 0 or end <= start or (asset.duration_sec is not None and end > asset.duration_sec):
            raise ValueError("inspect_range 时间范围非法")
    ...
    if action.name == "set_categories":
        categories = [ContentCategory.model_validate(item) for item in action.arguments.get("categories", [])]
