...
- 背景：视频首次导入 Eagle 时会使用“原文件名主干 + 中文内容短标题”的描述名，但 Eagle `item/update` API 不支持修改已有条目的名称。
- 想法：重新分析导致目标名称变化时，仅重建对应 Eagle 条目，而不是要求整个项目执行 `--reset`。
- 待确认：
  - 如何保留用户只在 Eagle 中手工添加的标签、备注、评分和文件夹归属。
  - 如何可靠比较上次同步名称与当前目标名称，避免无意义重建。
  - 移入回收站与重新导入之间失败时，如何恢复并避免条目丢失或重复。
...
- 低优先级 TODO：提升人声片段时间戳准确性。当前 Gemini 可能返回合法但与原音频不对齐的时间范围（例如模型给出 `0.6s`，人工试听约在 `9s`）。后续评估带词级时间戳的专用 ASR，或通过本地 VAD 检测语音区间并恢复原视频偏移；在此之前将模型时间戳视为粗定位，不承诺秒级准确。
