...
- 想法：在 analyze 产出的标签、摘要、分类和结构化字段之间增加一层语义冲突检测与消歧，避免同一素材出现互相打架的结论。
- 期望产出：
  - 自动识别常见冲突，例如 `subject_type`、`scene`、`summary`、标签之间前后矛盾或粒度不一致
  - 为冲突结果打标并记录原因，例如模型不一致、字段覆盖冲突、置信度接近
  - 在可能的情况下做自动消歧，例如按字段优先级、置信度、规则库或二次判断模型统一结果
  - 无法自动解决时，把冲突信息写回 `cut_index.json`，供 review 页面或人工修正使用
- 待确认：
  - 哪些字段组合算“语义冲突”，需要先定义一版规则清单
  - 是在单次 analyze 内解决，还是允许多轮 analyze 结果之间做对账
  - 自动消歧的优先顺序是规则优先、模型优先，还是人工优先
  - 冲突结果对下游导出、聚类、检索要不要做硬拦截
