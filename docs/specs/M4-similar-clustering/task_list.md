...
- [x] Task 1：扩展数据契约（[`src/tripclipper/models.py`](../../../src/tripclipper/models.py)）
  - [x] SubTask 1.1：新增 `ClusteringInfo` BaseModel，字段对齐 `AnalysisInfo`：
  ...
  - [x] SubTask 1.2：在 `CutIndex` 上加 `clustering: Optional[ClusteringInfo] = None`；更新 `__all__`
  - [x] SubTask 1.3：补单测：用现有 `cut_index.json` 反序列化时（没有 `clustering` 字段）`clustering=None`、其他字段不变

- [x] Task 2：实现本地启发式聚组与候选池生成（[`src/tripclipper/clusterer.py`](../../../src/tripclipper/clusterer.py)）
  - [x] SubTask 2.1：定义模块级常量 `_TIME_WINDOW_SECONDS = 60`、`_SUBJECT_SIMILARITY_THRESHOLD = 0.6`、
  ...
  - [x] SubTask 2.2：`_text_jaccard_2gram(s1: str, s2: str) -> float`——2-gram Jaccard 纯函数；
        中文 1 char = 1 token，按字符切 2-gram；空串/单字直接返回 0
  - [x] SubTask 2.3：`_tags_jaccard(t1: list[str], t2: list[str]) -> float`——集合 Jaccard；空集合返回 0
  - [x] SubTask 2.4：`_similarity_signals(a: Asset, b: Asset) -> tuple[bool, str]`——返回 (是否相似, 命中轨道说明)；
  ...
  - [x] SubTask 2.5：`_UnionFind` 私有类（标准并查集：`find` / `union` / `groups`）
  - [x] SubTask 2.6：`cluster_candidates(assets: list[Asset]) -> list[CandidateGroup]`——
  ...
  - [x] SubTask 2.7：`apply_similarity_states(cut: CutIndex, groups: list[SimilarGroup]) -> None`——
  ...
  - [x] SubTask 2.8：`build_default_candidates(cut: CutIndex) -> list[DefaultCandidate]`——
  ...
  - [x] SubTask 2.9：未 analyzed 素材统一标 `edit_candidate_status="needs_review"`、`edit_candidate_reason="分析未完成，待人工确认"`；
        needs_review 雷同组员标 `edit_candidate_reason="所在雷同组置信度不足，待人工确认"`

- [x] Task 3：实现组级 LLM 仲裁（[`src/tripclipper/arbiter.py`](../../../src/tripclipper/arbiter.py)）
  - [x] SubTask 3.1：定义 `ArbiterError(Exception)`、`ArbitrationResult` dataclass
  ...
  - [x] SubTask 3.2：模块级 `_ARBITER_SYSTEM_PROMPT_TEMPLATE` 常量——
  ...
  - [x] SubTask 3.3：`Arbiter.__init__(config: ModelConfig, editing_intent: EditingIntent)`——
  ...
  - [x] SubTask 3.4：`_build_group_user_content(group_assets: list[Asset]) -> list[dict]`——
        每条素材只送 `thumbnail_path` 的 base64 `image_url`；附带文本"asset_id={id}, rating={r}, subject_type={s}"
        作为 anchor，避免模型混淆素材身份
  - [x] SubTask 3.5：`arbitrate(group: list[Asset]) -> ArbitrationResult`——
  ...
  - [x] SubTask 3.6：`_parse_arbitration(text: str) -> ArbitrationResult`——纯函数。
  ...
  - [x] SubTask 3.7：复用 `provider._should_retry` 纯函数（直接 import 或镜像同义实现，但**不**抄 prompt 解析）；
        重试策略：可重试错误重试 1 次（共 2 次尝试，对应 spec Q16），间隔 1s + 抖动；非瞬时不重试
  - [x] SubTask 3.8：失败兜底——`arbitrate` 内部异常（瞬时全部失败 / JSON 非法 / 校验失败）一律向上抛分类异常；
        由 `cluster_runner.py` 统一处理 `arbitration_failures += 1`

- [x] Task 4：实现 cluster 流程编排（[`src/tripclipper/cluster_runner.py`](../../../src/tripclipper/cluster_runner.py)）
  - [x] SubTask 4.1：定义 `ClusterResult` dataclass：
  ...
  - [x] SubTask 4.2：`cluster(slug, *, base_dir=None) -> ClusterResult`——
        ...
        11. 返回 `ClusterResult`
  - [x] SubTask 4.3：增量落盘加锁——单进程串行不需要锁，但要保证落盘原子（`tmp + rename`，沿用 [`cut_index.py`](../../../src/tripclipper/cut_index.py) 既有 writer）
  - [x] SubTask 4.4：`_summarize_errors(arbitration_failures, needs_review_count)` 生成 `error_summary` 中文文案
        （如 `"2 组仲裁失败、3 组置信度不足"`）

- [x] Task 5：扩展 JSONL 日志（[`src/tripclipper/logs.py`](../../../src/tripclipper/logs.py)）
  - [x] SubTask 5.1：新增 `cluster_log_path(slug, ts, base_dir)` 到 [`paths.py`](../../../src/tripclipper/paths.py)：
        `projects/<slug>/logs/cluster-<ts>.jsonl`
  - [x] SubTask 5.2：复用 `AnalyzeLogger` 或新增 `ClusterLogger`（建议复用现有 logger，加 `kind` 参数）；
  ...
  - [x] SubTask 5.3：脱敏：与 M3 Q24 同纪律，日志**绝不**含 API key、Authorization、完整 prompt、模型响应 `content`

- [x] Task 6：CLI 接线（[`src/tripclipper/cli.py`](../../../src/tripclipper/cli.py)）
  - [x] SubTask 6.1：`--stage` 枚举追加 `"cluster"`
  - [x] SubTask 6.2：在 `analyze` 命令中处理 `stage == "cluster"` 分支——
  ...
  - [x] SubTask 6.3：错误兜底——`ArbiterError`、`AnalyzerError`（cluster_runner 可能复用）以面向用户的清晰文案打印并 `sys.exit(非0)`；
        密钥**绝不**出现在错误文案

- [x] Task 7：扩展 run 编排（[`src/tripclipper/runner.py`](../../../src/tripclipper/runner.py)）
  - [x] SubTask 7.1：`RunResult` 新增 `cluster: Optional[ClusterResult]`
  - [x] SubTask 7.2：`run(slug, ...)` 在 full 完成后追加调用 `cluster_runner.cluster(slug, base_dir=base_dir)`；
        cluster 失败时记入 `result.notes`、不抛中断（与 M3 `run` 风格一致：分步硬卡由编排自身保证）
  - [x] SubTask 7.3：CLI `run` 命令的输出摘要补 `[cluster]` 一行（`groups=...  pool_size=...  arbitration_failures=...`）

- [x] Task 8：Unit 测试（[`tests/test_clusterer.py`](../../../tests/test_clusterer.py) + [`tests/test_arbiter.py`](../../../tests/test_arbiter.py) 的纯函数部分 + [`tests/test_cluster_runner.py`](../../../tests/test_cluster_runner.py)）
  - 测试约定：**零 mock**。喂手工构造字符串测 `_parse_arbitration` 不算 mock 模型——约束的是
    "不许伪造分析结果当真写回 `cut_index.json`"，不是"不许测纯函数"。
  - [x] SubTask 8.1：`_text_jaccard_2gram`——
        `("海边夕阳人物", "海边夕阳人物")` → 1.0；
        `("海边夕阳人物", "海边人物夕阳")` → 介于 0.0 与 0.6 之间（2-gram 数学：交 3 / 并 7 ≈ 0.43；语序大变化故意不命中语义轨）；
        `("海边", "山顶")` → 0.0；
        空串 / 单字 → 0.0
  - [x] SubTask 8.2：`_tags_jaccard`——
        `(["海", "夕阳"], ["海", "夕阳"])` → 1.0；
        `(["海", "夕阳"], ["海", "人物"])` → 1/3；
        `([], [])` → 0.0
  - [x] SubTask 8.3：`_similarity_signals` 强信号轨——
        构造两个 `Asset`，`modified_time` 差 30 秒、`subject_type=building`，断言相似命中"强信号轨"；
        差 90 秒不相似；`subject_type` 不一致不相似
  - [x] SubTask 8.4：`_similarity_signals` 语义信号轨——
        构造两个 `Asset`，`subject_type` + `shot_scale` 一致、`primary_subject` 文本相似 0.7，断言相似；
        `tags` Jaccard 0.6 也命中；两者都 < 阈值则不相似
  - [x] SubTask 8.5：`_UnionFind` 并查集传递闭包——
        A 与 B 相似、B 与 C 相似、A 与 C 不直接相似 → `{A, B, C}` 同组
  - [x] SubTask 8.6：`cluster_candidates` 过滤未 analyzed 素材——
        构造 10 个 `Asset` 中 3 个 `analysis_status="scanned"`、7 个 `analyzed`，断言聚组只对 7 个工作
  - [x] SubTask 8.7：`build_default_candidates` 主体×景别平衡触发分支——
        合成 30 条 `Asset`，其中 18 条 `(subject_type=building, shot_scale=wide)`、12 条其他类型，
        断言触发修剪、building+wide 桶占比 ≤ 50%、修剪降级总数 ≤ 6
  - [x] SubTask 8.8：`build_default_candidates` 不触发分支——
        合成 15 条 `Asset` 平均分布，断言 `len < 20` 不触发任何修剪、所有素材进 `default_selected`
  - [x] SubTask 8.9：`build_default_candidates` 修剪上限分支——
        合成 30 条 `Asset` 中 25 条全部 `(building, wide)`（极端集中），断言修剪 ≤ 6（占比 20%），
        剩余 19 条 building+wide 仍在 default_selected
  - [x] SubTask 8.10：`build_default_candidates` needs_review 不进池——
  ...
  - [x] SubTask 8.11：`_parse_arbitration` 合法 JSON / 非法 JSON / `primary_asset_id` 引用不在组内（被拒）/
        `confidence < 0.6` 整组 `needs_review` / `basis` 越界过滤
  - [x] SubTask 8.12：cluster 重跑清空——构造已有 `clustering.status="completed"` 的 `cut_index`，
        断言 cluster_runner 启动时清空所有 similar/edit_candidate 字段
  - [x] SubTask 8.13：增量落盘——构造 5 个候选组，断言 `cluster_runner` 写盘次数 ≥ 6（每组一次 + 阶段头尾）；
        测试对落盘函数注入"计数 callback"参数而非替换函数本体
  - [x] SubTask 8.14：失败记录三处——构造 Arbiter 抛瞬时异常的情形，
  ...
  - [x] SubTask 8.15：日志脱敏——给 logger 传入故意含 `Authorization: Bearer sk-xxx` 的事件 dict，
        断言落盘 JSONL 中**不**出现该字面值

- [x] Task 9：Integration 测试（真打模型，[`tests/test_arbiter.py`](../../../tests/test_arbiter.py) integration 部分 + [`tests/test_integration_m4.py`](../../../tests/test_integration_m4.py)）
  ...
  - [x] SubTask 9.1：`test_arbiter_real_model`——用 demo-scan 的三条天然连号样本
  ...
  - [x] SubTask 9.2：`test_full_pipeline_with_cluster`——端到端，**原地** [`projects/demo-scan/`](../../../projects/demo-scan/) 跑 cluster：
  ...
  - [x] SubTask 9.3：`test_cluster_rerun_clears_old_state`——
...
- [x] Task 10：CLI 测试（[`tests/test_cli_analyze.py`](../../../tests/test_cli_analyze.py) 扩展）
  - [x] SubTask 10.1：`tripclipper analyze demo-scan --stage cluster` → 退出码 0、stdout 含"cluster 完成"摘要；
        cut_index.json 已写 `clustering.status` 终态
  - [x] SubTask 10.2：`tripclipper analyze <未init> --stage cluster` → 退出码非 0、stderr 提示先 init / scan / analyze、不抛堆栈
  - [x] SubTask 10.3：`tripclipper analyze <已init未scan> --stage cluster` → 退出码非 0、stderr 提示先 analyze --stage full
  - [x] SubTask 10.4：`tripclipper analyze <仅 sample 完成> --stage cluster` → 退出码 0 但 stderr 含 warning
        "警告：仅基于 sample 阶段的 N 个素材聚组"
  - [x] SubTask 10.5：`tripclipper run demo-scan` → scan/sample/full 已就绪时仅跑 cluster；stdout 摘要含 `[cluster]` 一行

- [x] Task 11：依赖与忽略项
  - [x] SubTask 11.1：无新依赖（复用 `httpx` / `python-dotenv`）
  - [x] SubTask 11.2：`.gitignore` 中 `projects/*/logs/` 通配已覆盖 `cluster-*.jsonl`，无需修改
  - [x] SubTask 11.3：[`docs/specs/README.md`](../README.md) 更新 M4 状态为"实施中"/"已完成"
