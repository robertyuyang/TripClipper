...
提取完成后，Analyzer 直接读取标准化 16-bit PCM WAV 的样本统计。只有分块同时满足峰值不高于 `-75 dBFS` 且 RMS 不高于 `-80 dBFS` 时，才把它确定为近乎数字静音，生成一个成功的 `none` 分块并跳过 Provider。该门只处理结论高度确定的静音，不尝试把雨声、风声或发动机声分类为无人声；非静音分块继续交给 Gemini。
...
模型也可能返回结构合法但明显属于格式垃圾的文本。Parser 因此执行不依赖外部模型的最小文本校验：纯空白或纯标点的非空结果，以及整体匹配 `MM:SS`、`HH:MM:SS` 并可带小数的时间码，不得作为清晰口语证据。无效片段被丢弃并记录 warning；仍有其他片段时保留模型分类，否则把 `clear` 降为 `unclear`。纯数字、`哈哈哈` 和重复拟声词可能是真实口语、笑声或哼唱，不由文本规则否决。该校验不把结果直接改为 `none`，因为文本无效不能证明音频中无人说话。
...
- [固定静音阈值可能误伤极低电平的真实人声] → 同时要求峰值和 RMS 低于非常保守的阈值，只处理接近数字底噪的分块；其他情况一律继续调用模型。
- [格式垃圾规则可能与真实口语文本重合] → 仅过滤纯标点和完整时间码，不过滤纯数字、笑声文本或拟声词，并降级为 `unclear` 而非 `none`。
